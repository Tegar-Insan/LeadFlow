...
import { getAnthropicClient, ANTHROPIC_MODEL } from "../config/anthropic.js";
import { supabaseAdmin as supabase } from "../config/supabase.js";
import logger from "../utils/logger.js";
import { retryWithBackoff, ANTHROPIC_RETRY } from "../utils/retryHelper.js";
import { requestIdeaImage } from "../services/imageGenerationClient.js";
...
async function insertDraft(promptId, userId, d) {
...
// ---------------------------------------------------------------------------
export async function generateScheduleDraftsFromBrief(brief, userId) {
    ...
    try {
        ...
        const response = await retryWithBackoff(() => anthropic.messages.create({
            ...
            ...ANTHROPIC_RETRY,
            onRetry: (attempt, delayMs) => logger.warn(`[ContentIdea] rate-limited — retry ${attempt} in ${delayMs}ms`),
        });
...
// ---------------------------------------------------------------------------
export async function generateScheduleDraftsWithStepsFromBrief(brief, userId) {
    if (!brief || brief.trim().length < 5) {
        throw new Error('Brief must be at least 5 characters');
    }
    const promptRow = await Prompt.create({ promptText: brief, userId });
...
export async function listPendingIdeasForUser(userId) {
    const { data, error } = await supabase
        ...
        'estimated_duration, estimated_engagement, best_time_to_post_wib, category, ' +
    ...
    return rows.map((row) => ({
        ...
        estimated_engagement: row.estimated_engagement ?? 'medium',
        best_time_to_post_wib: row.best_time_to_post_wib ?? '',
...
// ---------------------------------------------------------------------------
export async function listByPromptId(promptId) {
    const { data, error } = await supabase
        .from('content_ideas')
...
// ---------------------------------------------------------------------------
export async function approveIdea(ideaId, userId) {
    ...
    if (!data) {
        const e = new Error('Idea not in pending_validation state');
        e.statusCode = 409;
        throw e;
    }
...
export async function rejectIdea(ideaId, userId, rejectedReason) {
...
//# sourceMappingURL=ContentIdea.js.map