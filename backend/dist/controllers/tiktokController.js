...
function resolvePublishStatusCode(message = '') {
    ...
    if (text.includes('no tiktok account connected'))
        return 400;
    if (text.includes('tik tok account is not connected') || text.includes('tiktok account is not connected'))
        return 409;
    ...
    if (text.includes('unaudited_client_can_only_post_to_private_accounts'))
        return 403;
    if (text.includes('content-sharing-guidelines') || text.includes('integration guidelines'))
        return 403;
...
exports.handleCallback = async (req, res) => {
    ...
    const redirect = (slug) => res.redirect(`${TIKTOK_CONFIG.frontendUrl}/tiktok/callback?tiktok=${slug}`);
    if (tiktokErr)
        return redirect(`error&reason=${encodeURIComponent(tiktokErr)}`);
    if (!code || !state)
        return redirect('error&reason=missing_params');
    let userId;
    let codeVerifier;
    try {
