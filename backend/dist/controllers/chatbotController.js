...
// ─────────────────────────────────────────────────────────────────
// POST /api/chatbot/agent/generate-schedule
// Body: { brief: string }
// AI agent flow (non-chat): generate concrete recommendation and create schedule directly.
// ─────────────────────────────────────────────────────────────────
const generateScheduleAgent = async (req, res) => {
    try {
        const brief = String(req.body?.brief || '').trim();
        const userId = req.user?.userId || req.user?.id;
        if (!brief) {
            return error(res, { message: 'brief is required', statusCode: 400 });
        }
        if (!userId) {
            return error(res, { message: 'authentication is required', statusCode: 401 });
        }
        const prompt = [
        ...
        if (!schedule) {
            return error(res, {
                message: visibleText || 'AI could not generate a concrete schedule. Please provide a more specific brief.',
                statusCode: 422,
            });
        }
        const newSchedule = await createSchedule({
        ...
        logger.info(`[ai-agent] Schedule generated and created: ${newSchedule.id} by user ${userId}`);
        return success(res, {
        ...
        logger.error('[chatbotController.generateScheduleAgent]', err.message);
        return error(res, { message: 'Failed to generate schedule with AI agent', statusCode: 500 });
    }
};
module.exports = { sendMessage, approveSchedule, rejectSchedule, generateScheduleAgent };
