 ...
 */
export async function create(userId: string, photoUrl: string, storagePath: string): Promise<Record<string, unknown>> {
 ...
 */
export async function findAllByUserId(userId: string): Promise<Record<string, unknown>[]> {
  const { data, error } = await db
    .from('user_photos')
    .select('*')
    .eq('user_id', userId)
