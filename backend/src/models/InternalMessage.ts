 ...
 */
async function fallbackGetConversations(userId: string): Promise<Record<string, unknown>[]> {
  const { data, error } = await db
    .from('internal_messages')
    .select(
...
export async function sendMessage(
  ...
  if (error) throw new Error(`InternalMessage.sendMessage: ${error.message}`);
  return data;
}
...
export async function markAsRead(messageId: string): Promise<void> {
  ...
  if (error) throw new Error(`InternalMessage.markAsRead: ${error.message}`);
}

/**
 * Get message by ID
 */
export async function getMessageById(messageId: string): Promise<Record<string, unknown> | null> {
  const { data, error } = await db
    .from('internal_messages')
    .select('id, sender_id, receiver_id, message_text, is_read, created_at, updated_at')
    .eq('id', messageId)
...
export async function deleteMessage(messageId: string): Promise<void> {
  ...
  if (error) throw new Error(`InternalMessage.deleteMessage: ${error.message}`);
}
