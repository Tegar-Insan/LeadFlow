...
import * as UserPhoto from '../models/UserPhoto.ts';
...
export async function uploadPhoto(req, res) {
  try {
    ...
    if (activePhoto?.storage_path) {
      ...
      if (rmErr) logger.warn('[profileController] Old photo removal failed', rmErr);
    }
...
export async function deletePhoto(req, res) {
  try {
    ...
    const { error: rmErr } = await supabaseStorage
      .storage.from(PHOTO_BUCKET)
      .remove([activePhoto.storage_path]);
    if (rmErr) logger.warn('[profileController] Storage delete warning', rmErr);
