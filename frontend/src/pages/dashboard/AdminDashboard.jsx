...
function StatCard({ label, value, color = 'brand', icon }) {
  ...
  return (
    <div className="card p-5 flex items-start gap-4 animate-slide-up">
      <div className={`w-10 h-10 rounded-xl border flex items-center justify-center shrink-0 ${c}`}>{icon}</div>
      <div>
    ...
    <div className="min-h-screen bg-[#0a0a0a] flex">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="flex-1 p-6 animate-fade-in">
          {/* Header */}
          <div className="mb-8">
            <p className="text-text-muted text-sm font-mono mb-1">{nowJakarta().format('dddd, DD MMMM YYYY · HH:mm [WIB]')}</p>
            <h1 className="font-display font-bold text-2xl text-text-primary">Admin Panel</h1>
            <p className="text-text-secondary text-sm mt-1">System overview — LeadFlow / Krench Chicken</p>
          </div>

          {/* Stats */}
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            <StatCard label="Total Users"      value={loading ? '…' : stats.total}  color="brand"
              icon={<svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z"/></svg>}
            />
            <StatCard label="Active Users"     value={loading ? '…' : stats.active} color="green"
              icon={<svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z"/></svg>}
            />
            <StatCard label="Business Owners"  value={loading ? '…' : stats.owners} color="amber"
            ...
            />
            <StatCard label="Marketing Staff"  value={loading ? '…' : stats.staff}  color="sky"
                        ...
                        <tr key={u.userid} className="border-b border-surface-border last:border-0 hover:bg-surface-raised/50 transition-colors">
                          <td className="py-3 px-4">
                            <div className="flex items-center gap-3">
                              <div className="w-7 h-7 rounded-lg bg-brand/10 border border-brand/20 flex items-center justify-center text-brand text-xs font-bold font-display">
