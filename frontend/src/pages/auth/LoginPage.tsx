...
// Redesigned — "Digital Growth Login"
    ...
    <div style={styles.body}>
      <div style={styles.loaderWrap}>
        <div style={styles.dots}>
          {[0, 1, 2, 3, 4].map((i) => (
            <div key={i} className={`dot dot-${i}`} style={styles.dot} />
          ))}
        </div>
      ...
      <style>{`
        ...
        .progress-fill { animation: progress 2.4s ease-in-out infinite; }
      `}</style>
    </div>
  );
}
...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (isAuthenticated && loginDone) {
      const dest = from || dashboardPath || '/dashboard';
