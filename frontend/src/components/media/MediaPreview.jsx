...
const getAssetUrl = (asset) => asset?.file_url
  || asset?.url
  || asset?.preview_url
  || asset?.signed_url
  || buildMediaUrl(asset?.id || asset?.asset_id)
  || '';
...
const getAssetKind = (asset) => {
  ...
  const sourceName = String(asset?.file_name || asset?.name || asset?.url || asset?.file_url || '').toLowerCase();
